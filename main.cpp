// Travel Agency Management System Project

#include<iostream>
#include<cstdio>
#include<fstream>
#include<sstream>
#include<string>
#include<cstdlib>
#include<conio.h>
#include<windows.h>
#include<mysql.h>
#include<math.h>
using namespace std;

// Global Variable initiated
int qstate;
MYSQL* conn;
MYSQL_ROW row;
MYSQL_RES* res;
// Global Variable End

class db_response
{
public:
    static void ConnectionFunction()
    {
        conn = mysql_init(0);
        if (conn)
        {
            cout << "Database Connected" << endl;
            cout << "Press any key to continue..." << endl;
            //getch();
            system("cls");
        }
        else
            cout << "Failed To Connect!" << mysql_errno(conn) << endl;
        conn = mysql_real_connect(conn, "localhost", "admin", "admin", "travel_db", 0, NULL, 0);
        if (conn)
        {
            cout << "Database Connected To MySql" << conn << endl;
            cout << "Press any key to continue..." << endl;
            //getch();
            system("cls");
        }
        else
            cout << "Failed To Connect!" << mysql_errno(conn) << endl;
    }
};



//Functions Initialization
void NewTravelTrip();
void ShowUsers();
void FindUser();
void EditUser();
void DeleteUser();
void AddTrip();
void EditTrip();
void DeleteTrip();
//Functions ended

int main()
{
    // Initial Load
    system("cls");
    system("title Travel Agency Management System Program");
    system("color 0f");
    // Initial Load End

    // Call Methods
    db_response::ConnectionFunction();
    // Call Methods End
    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;



    // Variables
    int chooseOneFromMenu = 0;
    char exitSurity;
    // Variables End

    cout << "             --------------------------------------------------------" << endl;
    cout << "             |           Travel Agency Management Menu              |"  << endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "                   1---> New User Travel Trip Plan.\t" << endl;
    cout << "                   2---> Show Users.\t" << endl;
    cout << "                   3---> Find User.\t" << endl;
    cout << "                   4---> Edit User.\t" << endl;
    cout << "                   5---> Delete User.\t" << endl;
    cout << "                   6---> Add Trip.\t" << endl;
    cout << "                   7---> Edit Trip.\t" << endl;
    cout << "                   8---> Delete Trip.\t" << endl;
    cout << "                   9---> Exit the Console.\t" << endl;
    cout << "             ---------------------------------------------------------" << endl << endl << endl ;
    cout << "                            Press the Navigation digit: ";
    cin >> chooseOneFromMenu;

    switch (chooseOneFromMenu)
    {
    case 1:
        NewTravelTrip();
        break;
    case 2:
        ShowUsers();
        break;
    case 3:
        FindUser();
        break;
    case 4:
        EditUser();
        break;
    case 5:
        DeleteUser();
        break;
    case 6:
        AddTrip();
        break;
    case 7:
        EditTrip();
        break;
    case 8:
        DeleteTrip();
        break;
    case 9:
        ExitProgram:
        cout << "Program terminating. Are you sure? (y/N): ";
        cin >> exitSurity;
        if (exitSurity == 'y' || exitSurity == 'Y') {
            return 0;
        }else if (exitSurity == 'n' || exitSurity == 'N') {
            system("cls");
            main();
        }else {
            cout << "Next time choose after read the corresponding line." << endl;
            goto ExitProgram;
        }
        break;
    default:
        cout << "Please choose between 1 - 7. Press Enter To Continue..."<<endl;
        getch();
        system("cls");
        main();
        break;
    }
    return 0;
}

void NewTravelTrip()
{

     // Initial Load
    system("cls");
    // Initial Load End

    // Variables
    string userName = "";
    string userAge = "";
    string userSex = "";
    string userLocation = "";
    string userContact = "";
    string userEmail = "";
    string userTripDate = "";
    string userTripPlanId = "";
    string userTripPlan = "";
    string userTripCost = "";
    char choose;
    // Variables End

    // Store Variables
    string storeUserTripPlan[100][3];
    int storeIndex1 = 0, storeIndex2 = 0;
    // Store Variables End

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                 New Travel Trip Menu                 |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;
    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "New Travel Trip Menu" << endl << endl;

    cin.ignore(1, '\n');
    cout << "Enter User Name: ";
    getline(cin, userName);
    cout << "Enter Age: ";
    getline(cin, userAge);
    cout << "Enter Sex: ";
    getline(cin, userSex);
    cout << "Enter Location: ";
    getline(cin, userLocation);
    cout << "Enter Contact No.: ";
    getline(cin, userContact);
    cout << "Enter Email: ";
    getline(cin, userEmail);
    cout << "Enter Trip Date: ";
    getline(cin, userTripDate);
    qstate = mysql_query(conn, "select * from trips_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("----------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-40s | %-40s |\n", row[0], row[1],row[2]);
            storeUserTripPlan[storeIndex1][storeIndex2] = row[0];
            storeIndex2++;
            storeUserTripPlan[storeIndex1][storeIndex2] = row[1];
            storeIndex2++;
            storeUserTripPlan[storeIndex1][storeIndex2] = row[2];
            storeIndex1++;storeIndex2--;storeIndex2--;
        }
        printf("----------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }
    cout << "Enter Trip Id: ";
    getline(cin, userTripPlanId);
    for (int i = 0, j = 0; i < storeIndex1; i++)
    {
        if (storeUserTripPlan[i][j] == userTripPlanId)
        {
            userTripPlan = storeUserTripPlan[i][1];
            userTripCost = storeUserTripPlan[i][2];
            break;
        }
    }

    string insert_query = "insert into agency_users_tb (user_name, user_age, user_sex, user_location, user_contact, user_email,user_trip_date, user_trip_plan, user_trip_cost) values ('"+userName+"', '"+userAge+"', '"+userSex+"', '"+userLocation+"','"+userContact+"','"+userEmail+"','"+userTripDate+"','"+userTripPlan+"','"+userTripCost+"')";

    const char* q = insert_query.c_str(); // c_str converts string to constant char and this is required

    qstate = mysql_query(conn, q);

    if (!qstate)
    {
        cout << endl << "Successfully added in database." << endl;
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    // Exit Code
    cout << "Press 'm' to Menu and 'a' to Insert Again Or Any Other key to exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'a' || choose == 'A')
    {
        NewTravelTrip();
    }
    else
    {
        exit(0);
    }
}

void ShowUsers()
{
     // Initial Load
    system("cls");
    // Initial Load End

    // Variables
    char choose;
    // Variables End

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                Show Agency's Users Menu               |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;
    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Show Users Menu" << endl << endl;

    qstate = mysql_query(conn, "select * from agency_users_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        while ((row = mysql_fetch_row(res)))
        {
            cout << "User Name: " << row[1] << "\nAge: " << row[2] << "\nSex : " << row[3] << "\nUser Address: " << row[4] << "\nContact : " << row[5] << "\nEmail: " << row[6] << "\nTrip Date: " << row[7] << "\nTrip Plan: " << row[8] << "\nTrip Cost : " << row[9] << endl << endl;
        }
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    // Exit Code
    ExitMenu:
    cout << "Press 'm' to Menu and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else
    {
        exit(0);
    }
}

void FindUser()
{
    // Initial Load
    system("cls");
    // Initial Load End

    // Variables
    string input = "";
    char choose;
    // Variables End

    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Show Users Menu" << endl << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |              Find Agency's Users Menu                 |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    cin.ignore(1, '\n');
    cout << "Enter User Name: ";
    getline(cin, input);
    string findbyname_query = "select * from agency_users_tb where user_name like '%"+input+"%'";
    const char* qn = findbyname_query.c_str();
    qstate = mysql_query(conn, qn);

    cout << endl;
    if (!qstate)
    {
        res = mysql_store_result(conn);
        while ((row = mysql_fetch_row(res)))
        {
            cout << "User Name: " << row[1] << "\nAge: " << row[2] << "\nSex : " << row[3] << "\nUser Address: " << row[4] << "\nContact : " << row[5] << "\nEmail: " << row[6] << "\nTrip Date: " << row[7] << "\nTrip Plan: " << row[8] << "\nTrip Cost : " << row[9] << endl << endl;
        }
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    ExitMenu:
    cout << "Press 'm' to Menu, 'a' to Search again and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'a' || choose == 'A')
    {
        FindUser();
    }
    else
    {
        exit(0);
    }
}

void EditUser()
{
    system("cls");

    // Variables
    string userName = "";
    string userAge = "";
    string userSex = "";
    string userLocation = "";
    string userContact = "";
    string userEmail = "";
    string userTripDate = "";
    string userTripPlanId = "";
    string userTripPlan = "";
    string userTripCost = "";
    string items[5000];
    char choose;
    int itemId;
    bool HaveException = false;
    bool NotInDatabase = false;
    int indexForId = 0;

    // Store Variables
    string storeColumnId = "";
    string storeUserName = "";
    string storeUserAge = "";
    string stroreUserSex = "";
    string storeUserLocation = "";
    string storeUserContact = "";
    string storeUserEmail = "";
    string storeUserTripDate = "";
    string storeUserTripPlanId = "";
    string storeUserTripPlan = "";
    string storeUserTripCost = "";
    string storeUserTripPlan2d[100][3];
    int storeIndex1 = 0, storeIndex2 = 0;
    // Variables End

   // cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Edit User Trip Menu" << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |               Edit User's Travel Trip Menu            |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    qstate = mysql_query(conn, "select * from agency_users_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("-----------------------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-25s | %-25s | %-40s |\n", "Column ID", "User Name", "Trip Date", "Trip Plan");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-25s | %-25s | %-40s |\n", row[0], row[1], row[7], row[8]);
        }
        printf("-----------------------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    try
    {
        cout << endl;
        cout << "Enter Item Column ID: ";
        cin >> itemId;
        cout << endl;
    }
    catch (exception e)
    {
        cout << "Please Enter a valid NUMBER." << endl;
        HaveException = true;
        goto ExitMenu;
    }

    if (HaveException == false)
    {
        stringstream streamid;
        string strid;
        streamid << itemId;
        streamid >> strid;

        for (int i = 0; i < indexForId; i++)
        {
            if (strid != items[i])
            {
                NotInDatabase = true;
            }
            else
            {
                NotInDatabase = false;
                break;
            }
        }

        if (NotInDatabase == false)
        {
            string findbyid_query = "select * from agency_users_tb where user_id = '"+strid+"'";
            const char* qi = findbyid_query.c_str();
            qstate = mysql_query(conn, qi);

            if (!qstate)
            {
                cout << endl;

                res = mysql_store_result(conn);
                while ((row = mysql_fetch_row(res)))
                {
                   cout << "User Name: " << row[1] << "\nAge: " << row[2] << "\nSex : " << row[3] << "\nUser Address: " << row[4] << "\nContact : " << row[5] << "\nEmail: " << row[6] << "\nTrip Date: " << row[7] << "\nTrip Plan: " << row[8] << "\nTrip Cost : " << row[9] << endl << endl;

                    storeColumnId = row[0];
                    storeUserName = row[1];
                    storeUserAge = row[2];
                    stroreUserSex = row[3];
                    storeUserLocation = row[4];
                    storeUserContact = row[5];
                    storeUserEmail = row[6];
                    storeUserTripDate = row[7];
                    storeUserTripPlan = row[8];
                    storeUserTripCost = row[9];
                }
            }
            else
            {
                cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
            }

            cin.ignore(1, '\n');
            cout << "Enter User Name (X to not change): ";
            getline(cin, userName);
            if (userName == "X")
            {
                userName = storeUserName;
            }
            cout << "Enter Age (X to not change): ";
            getline(cin, userAge);
            if (userAge == "X")
            {
                userAge = storeUserAge;
            }
            cout << "Enter Sex (X to not change): ";
            getline(cin, userSex);
            if (userSex == "X")
            {
                userSex = stroreUserSex;
            }
            cout << "Enter User Location (X to not change): ";
            getline(cin, userLocation);
            if (userLocation == "X")
            {
                userLocation = storeUserLocation;
            }
            cout << "Enter User Contact No. (X to not change): ";
            getline(cin, userContact);
            if (userContact == "X")
            {
                userContact = storeUserContact;
            }
            cout << "Enter User Email (X to not change): ";
            getline(cin, userEmail);
            if (userEmail == "X")
            {
                userEmail = storeUserEmail;
            }
            cout << "Enter User Trip Date (X to not change): ";
            cin >> userTripDate;
            if (userTripDate == "X")
            {
                userTripDate = storeUserTripDate;
            }

            qstate = mysql_query(conn, "select * from trips_tb");
            if (!qstate)
            {
                res = mysql_store_result(conn);
                printf("----------------------------------------------------------------------------------------------------\n");
                printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
                while ((row = mysql_fetch_row(res)))
                {
                    printf("| %-10s | %-40s | %-40s |\n", row[0], row[1],row[2]);
                    storeUserTripPlan2d[storeIndex1][storeIndex2] = row[0];
                    storeIndex2++;
                    storeUserTripPlan2d[storeIndex1][storeIndex2] = row[1];
                    storeIndex2++;
                    storeUserTripPlan2d[storeIndex1][storeIndex2] = row[2];
                    storeIndex1++;storeIndex2--;storeIndex2--;
                }
                printf("----------------------------------------------------------------------------------------------------\n");
            }
            else
            {
                cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
            }
            cin.ignore(1, '\n');
            cout << "Enter Trip Id (X to not change): ";
            getline(cin, userTripPlanId);
            if (userTripPlanId == "X")
            {
                userTripPlan = storeUserTripPlan;
            }
            else
            {
                for (int i = 0, j = 0; i < storeIndex1; i++)
                {
                    if (storeUserTripPlan2d[i][j] == userTripPlanId)
                    {
                        userTripPlan = storeUserTripPlan2d[i][1];
                        userTripCost = storeUserTripPlan2d[i][2];
                        break;
                    }
                }
            }

            string update_query = "update agency_users_tb set user_name = '"+userName+"', user_age = '"+userAge+"', user_sex = '"+userSex+"', user_location = '"+userLocation+"', user_contact = '"+userContact+"', user_email = '"+userEmail+"', user_trip_date = '"+userTripDate+"', user_trip_plan = '"+userTripPlan+"', user_trip_cost = '"+userTripCost+"' where user_id = '"+strid+"'";

            const char* qu = update_query.c_str();
            qstate = mysql_query(conn, qu);

            if (!qstate)
            {
                cout << endl << "Successfully Saved In Database." << endl;
            }
            else
            {
                cout << "Failed To Update!" << mysql_errno(conn) << endl;
            }

        }
        else
        {
            cout << "Item Not Found in database." << endl;
        }
    }

    ExitMenu:
    cout << "Press 'm' to Menu, 'e' to edit another item and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'e' || choose == 'E')
    {
        EditUser();
    }
    else
    {
        exit(0);
    }
}

void DeleteUser()
{
    system("cls");

    // Variables
    char choose;
    int itemId;
    string items[5000];
    int indexForId = 0;
    bool HaveException = false, NotInDatabase = false;
    // Variables End

    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Delete Users Menu" << endl << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                   Delete Users Menu                  |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    qstate = mysql_query(conn, "select * from agency_users_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("-----------------------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-25s | %-25s | %-40s |\n", "Column ID", "User Name", "Trip Date", "Trip Plan");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-25s | %-25s | %-40s |\n", row[0], row[1], row[7], row[8]);
            items[indexForId] = row[0];
            indexForId++;
        }
        printf("-----------------------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    try
    {
        cout << endl;
        cout << "Enter Item Column ID: ";
        cin >> itemId;
        cout << endl;
    }
    catch (exception e)
    {
        cout << "Please Enter a valid NUMBER." << endl;
        HaveException = true;
        goto ExitMenu;
    }

    if (HaveException == false)
    {
        stringstream streamid;
        string strid;
        streamid << itemId;
        streamid >> strid;

        for (int i = 0; i < indexForId; i++)
        {
            if (strid != items[i])
            {
                NotInDatabase = true;
            }else
            {
                NotInDatabase = false;
                break;
            }
        }

        if (NotInDatabase == false)
        {
            string delete_query = "delete from agency_users_tb where user_id = '"+strid+"'";
            const char* qd = delete_query.c_str();
            qstate = mysql_query(conn, qd);

            if (!qstate)
            {
                cout << "Successfully Deleted From Database." << endl;
            }
            else
            {
                cout << "Failed To Delete!" << mysql_errno(conn) << endl;
            }

        }
        else
        {
            cout << "Item Not Found in database." << endl;
        }
    }

    // Exit Code
    ExitMenu:
    cout << "Press 'm' to Menu, 'd' to delete another record and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'd' || choose == 'D')
    {
        DeleteUser();
    }
    else
    {
        exit(0);
    }
}

void AddTrip()
{
    // Initial Load
    system("cls");
    // Initial Load End

    // Variables
    string tripName = "";
    string tripcost = "";
    char choose;
    // Variables End

    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Add Trip Menu" << endl << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                 Add Travel Trip Menu                 |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    cin.ignore(1, '\n');
    cout << "Enter Trip Name: ";
    getline(cin, tripName);
    cout<< "Enter Trip's Cost: ";
    getline(cin, tripcost);

    string insert_query = "insert into trips_tb (trip_name,trip_cost) values ('"+tripName+"','"+tripcost+"')";

    const char* q = insert_query.c_str(); // c_str converts string to constant char and this is required

    qstate = mysql_query(conn, q);

    if (!qstate)
    {
        cout << endl << "Successfully added in database." << endl;
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    qstate = mysql_query(conn, "select * from trips_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("----------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-40s | %-40s |\n", row[0], row[1], row[2]);
        }
        printf("----------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    // Exit Code
    cout << "Press 'm' to Menu and 'a' to Insert Again Or Any Other key to exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'a' || choose == 'A')
    {
        AddTrip();
    }
    else
    {
        exit(0);
    }
}

void EditTrip()
{
    system("cls");

    // Variables
    string tripName = "";
    string tripCost = "";
    string items[5000];
    char choose;
    int itemId;
    bool HaveException = false;
    bool NotInDatabase = false;
    int indexForId = 0;

    // Store Variables
    string storeColumnId = "";
    string storeTripName = "";
    // Variables End

    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Edit Trip Record" << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                 Edit Trip Record Menu                 |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    qstate = mysql_query(conn, "select * from trips_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("----------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-40s | %-40s |\n", row[0], row[1], row[2]);
        }
        printf("----------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    try
    {
        cout << endl;
        cout << "Enter Item Column ID: ";
        cin >> itemId;
        cout << endl;
    }
    catch (exception e)
    {
        cout << "Please Enter a valid NUMBER." << endl;
        HaveException = true;
        goto ExitMenu;
    }

    if (HaveException == false)
    {
        stringstream streamid;
        string strid;
        streamid << itemId;
        streamid >> strid;

        for (int i = 0; i < indexForId; i++)
        {
            if (strid != items[i])
            {
                NotInDatabase = true;
            }else
            {
                NotInDatabase = false;
                break;
            }
        }

        if (NotInDatabase == false)
        {
            string findbyid_query = "select * from trips_tb where trip_id = '"+strid+"'";
            const char* qi = findbyid_query.c_str();
            qstate = mysql_query(conn, qi);

            if (!qstate)
            {
                cout << endl;
                res = mysql_store_result(conn);
                printf("----------------------------------------------------------------------------------------------------\n");
                printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
                while ((row = mysql_fetch_row(res)))
                {
                    printf("| %-10s | %-40s | %-40s |\n", row[0], row[1], row[2]);
                }
                printf("----------------------------------------------------------------------------------------------------\n");
            }
            else
            {
                cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
            }

            cin.ignore(1, '\n');
            cout << "Enter Trip Name (xN to not change): ";
            getline(cin, tripName);
            if (tripName == "xN")
            {
                tripName = storeTripName;
            }
            else
            {
                cout<< "Enter Trip Cost: ";
                getline(cin, tripCost);
            }
            string update_query = "update trips_tb set trip_name = '"+tripName+"', trip_cost = '"+tripCost+"' where trip_id = '"+strid+"'";
            const char* qu = update_query.c_str();
            qstate = mysql_query(conn, qu);

            if (!qstate)
            {
                cout << endl << "Successfully Saved In Database." << endl;
            }
            else
            {
                cout << "Failed To Update!" << mysql_errno(conn) << endl;
            }

        }
        else
        {
            cout << "Item Not Found in database." << endl;
        }
    }

    ExitMenu:
    cout << "Press 'm' to Menu, 'e' to edit another item and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'e' || choose == 'E')
    {
        EditTrip();
    }
    else
    {
        exit(0);
    }
}

void DeleteTrip()
{
    system("cls");

    // Variables Declared
    char choose;
    int itemId;
    string items[5000];
    int indexForId = 0;
    bool HaveException = false, NotInDatabase = false;
    // Variables End

    //cout << "Welcome To Travel Agency Management System" << endl << endl;
    //cout << "Delete User Menu" << endl << endl;

    cout    <<"\t\t  *******************************************  " << endl<<endl
            <<"\t\t|| WELCOME TO TRAVEL AGENCY MANAGEMENT SYSTEM ||" << endl<<endl
            <<"\t\t  *******************************************  " << endl << endl<<endl<<endl;
    cout << "             --------------------------------------------------------" << endl;
    cout << "             |                   Delete Trip Menu                    |"  << endl;
    cout << "             --------------------------------------------------------" << endl << endl <<endl;

    qstate = mysql_query(conn, "select * from trips_tb");
    if (!qstate)
    {
        res = mysql_store_result(conn);
        printf("----------------------------------------------------------------------------------------------------\n");
        printf("| %-10s | %-40s | %-40s |\n", "Trip Id", "Trip Name", "Trip Cost");
        while ((row = mysql_fetch_row(res)))
        {
            printf("| %-10s | %-40s | %-40s |\n", row[0], row[1], row[2]);
        }
        printf("----------------------------------------------------------------------------------------------------\n");
    }
    else
    {
        cout << "Query Execution Problem!" << mysql_errno(conn) << endl;
    }

    try
    {
        cout << endl;
        cout << "Enter Item Column ID: ";
        cin >> itemId;
        cout << endl;
    }
    catch (exception e)
    {
        cout << "Please Enter a valid NUMBER." << endl;
        HaveException = true;
        goto ExitMenu;
    }

    if (HaveException == false)
    {
        stringstream streamid;
        string strid;
        streamid << itemId;
        streamid >> strid;

        for (int i = 0; i < indexForId; i++)
        {
            if (strid != items[i])
            {
                NotInDatabase = true;
            }else
            {
                NotInDatabase = false;
                break;
            }
        }

        if (NotInDatabase == false)
        {
            string delete_query = "delete from trips_tb where trip_id = '"+strid+"'";
            const char* qd = delete_query.c_str();
            qstate = mysql_query(conn, qd);

            if (!qstate)
            {
                cout << "Successfully Deleted From Database." << endl;
            }
            else
            {
                cout << "Failed To Delete!" << mysql_errno(conn) << endl;
            }

        }
        else
        {
            cout << "Item Not Found in database." << endl;
        }
    }

    // Exit Code
    ExitMenu:
    cout << "Press 'm' to Menu, 'd' to delete another record and any other key to Exit: ";
    cin >> choose;
    if (choose == 'm' || choose == 'M')
    {
        main();
    }
    else if (choose == 'd' || choose == 'D')
    {
        DeleteTrip();
    }
    else
    {
        exit(0);
    }
}
